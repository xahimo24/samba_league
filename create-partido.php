...
if ($result->num_rows > 0) {
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        foreach ($teamPlayers as $tp) {
            $sql = "INSERT INTO equipos_temporales (id_partido, color, id_jugador) VALUES ($partido_id, '{$tp['color']}', {$tp['playerId']})";
            $conn->query($sql);
        }
        ...
        // Insertar plantillas de equipos locales y visitantes
        $sql = "INSERT INTO Plantilla (id_equipo, id_jugador) 
                SELECT $id_equipo_local, id_jugador FROM equipos_temporales WHERE id_partido = $partido_id AND color = 'local'";
        $conn->query($sql);

        $sql = "INSERT INTO Plantilla (id_equipo, id_jugador) 
                SELECT $id_equipo_visitante, id_jugador FROM equipos_temporales WHERE id_partido = $partido_id AND color = 'visitante'";
        $conn->query($sql);
